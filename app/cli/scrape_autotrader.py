...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch and persist AutoTrader search/listing snapshots.")
...
async def run(args: argparse.Namespace) -> dict:
        ...
        if not settings.zyte_api_key:
            raise SourceScrapingError(
                SourceFailureDetail(
                    ...
                    message="ZYTE_API_KEY is required for live AutoTrader scraping. Use --fixture-mode for local fixtures.",
                    retryable=False,
                )
            )
        zyte_client = ZyteClient(api_key=settings.zyte_api_key, api_url=settings.zyte_api_url)

    adapter = AutoTraderAdapter(settings=settings, zyte_client=zyte_client)
    ...
    result: dict = {"source": "autotrader", "fixture_mode": fixture_mode}

    if args.url:
        listing_snapshot = await adapter.fetch_listing(SourceListingRef(source_name="autotrader", url=args.url))
        ...
        result.update({"listing_snapshot": _persisted_payload(persisted), "parsed_listing": _parsed_listing_payload(parsed)})
        return result
